<?php
    ...
    if(isset($_POST['insert_product'])){
        ...
        or $product_category='' or $product_price=''or $product_image=''){
            echo "<script>alert('Vui lòng điền vào mẫu sản phẩm ')</script>";
            exit();
        }else{  
            move_uploaded_file($_FILES['product_image']['tmp_name'], 
            'C:\xampp\htdocs\ShopSOS\admin\product_images/' . basename($_FILES['product_image']['tmp_name']));

            $insert_products="insert into `products` (product_title,product_description,product_keywords,
            product_category,product_image,product_price,date,status) 
